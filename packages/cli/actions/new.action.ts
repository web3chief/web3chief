...
import {Semantics} from "../lib/schematics/web3chief.collection";
...
export class NewAction extends AbstractAction {
  public async handle(inputs: Input[], options: Input[]) {
    ...
    const typeProject = await selectTypeApp();
    await generateApplicationFiles(inputs, options, typeProject).catch(exit);
...
const askForMissingInformation = async (inputs: Input[]) => {
  ...
  if (!nameInput!.value) {
    ...
    const questions = [generateInput('name', message)('web3chief-app')];
...
const generateApplicationFiles = async (args: Input[], options: Input[], typeProject: Semantics) => {
  const collectionName = options.find(
    (option) => option.name === 'collection' && option.value != null,
  )!.value;
  const collection: AbstractCollection = CollectionFactory.create(
    (collectionName as Collection) || Collection.WEB3CHIEF,
  );
  const schematicOptions: SchematicOption[] = mapSchematicOptions(
    args.concat(options),
  );
  await collection.execute(typeProject, schematicOptions);
  console.info();
};
...
const selectTypeApp = async (): Promise<Semantics> => {
  const answers: Answers = await askForTypeApp();
...
const askForTypeApp = async (): Promise<Answers> => {
  const questions: Question[] = [
    generateSelect('type-app')(MESSAGES.TYPE_APP_QUESTION)([
      Semantics.DEFAULT_APP,
    ]),
  ];
  const prompt = inquirer.createPromptModule();
  return await prompt(questions);
...
const printCollective = () => {
  ...
  emptyLine();
  yellow(`Thanks for installing web3chief`);
  dim('If you like web3chief please put a star on github');
  dim('to help us maintain this package.');
  emptyLine();
  emptyLine();
  print()(
    `${chalk.bold(`${EMOJIS.STAR}  Link:`)} ${chalk.underline(
      'https://github.com/web3chief/web3chief',
    )}`,
  );
  emptyLine();
};
