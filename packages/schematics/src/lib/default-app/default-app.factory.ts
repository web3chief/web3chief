...
import { DefaultOptions } from './default-app.schema';

export function main(options: DefaultOptions): Rule {
...
function transform(options: DefaultOptions): DefaultOptions {
    const target: DefaultOptions = Object.assign({}, options);
...
function generate(options: DefaultOptions, path: string): Source {
    return apply(url(join('./files' as Path, <string>options.language)), [
        template({
            ...strings,
            ...options,
        }),
